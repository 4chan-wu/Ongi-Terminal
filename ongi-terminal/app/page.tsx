...
export default function Home() {
  ...
  const [items, setItems] = useState<SharingItem[]>([
    {
      ...
      reportDesc: "오른쪽 하단부 미세 기스 존재하나 기능상 100% 정상 작동합니다.",
      explain: "작년에 선물 받아서 서너 번 사용하고 박스에 고이 모셔둔 무선 헤드폰입니다. 다른 헤드폰을 주로 쓰게 되어 이웃분들과 좋은 소리를 나누고자 나눔합니다.",
      location: "카페 앞 정거장",
      terminalId: "1",
      status: "available",
      category: "전자기기",
    ...
    },
    {
      ...
      reportDesc: "스페이스바 우측 키캡 생활 흠집 미세 존재, 동글이와 USB 케이블 포함.",
      explain: "주로 개발 코딩용으로 아껴 쓰던 키보드입니다. 이번에 기종 변경으로 더이상 쓰지 않아, 공부하는 학생분이나 개발자 이웃분들을 위해 가치있게 나눔합니다.",
      location: "주민센터 앞 정거장",
      terminalId: "2",
      status: "available",
      category: "전자기기",
    ...
    },
    {
      ...
      explain: "지난 가을 캠핑 때 두어 번 사용한 감성 가득 목재 LED 랜턴입니다. 집안 무드등이나 야외 활동 시 조명으로 매우 훌륭합니다.",
      location: "지하철역 앞 정거장",
      terminalId: "3",
      status: "available",
      category: "생활용품",
    ...
    },
    {
      ...
      reportDesc: "3권 책 모퉁이 미세 헤짐 외 낙서 일체 없이 깨끗한 소장용 상태.",
      explain: "아이가 부쩍 자라서 중학생이 된 바람에 더 이상 보지 않아 나눔합니다. 초등학교 저학년 아이들이 재미있는 그림을 보며 유익하게 학습하기에 아주 좋습니다.",
      location: "주민센터 앞 정거장",
      terminalId: "2",
      status: "available",
      category: "도서",
  ...
  const availableSharingCount = items.filter((item) => item.status === "available").length;
      ...
      <footer className="border-t border-brand-orange-light/30 bg-brand-ivory py-8 text-center text-xs font-semibold text-brand-gray">
        <div className="mx-auto max-w-7xl px-6 md:px-12 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex gap-4">
            <a href="#" className="hover:text-brand-orange transition-colors">이용약관</a>
            <a href="#" className="hover:text-brand-orange transition-colors">개인정보처리방침</a>
            <a href="#" className="hover:text-brand-orange transition-colors">지구 구하기</a>
          </div>
        </div>
      </footer>
    </div>
